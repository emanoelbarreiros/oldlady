...
class GameBoard {
    ...
    private fun getLine(line: Int): List<Play> = board[line]

    private fun getColumn(column: Int): List<Play> =
        listOf(board[0][column], board[1][column], board[2][column])

    private fun getMainDiagonal(): List<Play> = listOf(board[0][0], board[1][1], board[2][2])

    private fun getSecondaryDiagonal(): List<Play> = listOf(board[0][2], board[1][1], board[2][0])
